...
export function setOuterHTML ({ nodeId, outerHTML }) {
    ...
    let lastNodeId = elem.previousElementSibling._nodeId
...
export function setInspectedNode ({ nodeId }) {
    ...
    /**
     * for some reasons Samsung can't deal with having `'$'` in interpreted code
     * that is why we ignore eslint in that line
     */
