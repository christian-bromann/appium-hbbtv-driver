 ...
 */
export function getConsoleArg (arg, scriptId = 1) {
    ...
    const result = {
        ...
        description,
        objectId: `{"injectedScriptId": 1, "id": ${scriptId} }`, // ToDo replace with real objectId
        type
    }
...
export function getObjectProperties (obj, includeDescriptors = false) {
    ...
    return Object.getOwnPropertyNames(obj).map((propertyName) => {
        ...
        let value = JSON.stringify(descriptor.value)
        let className
        if (subtype === 'node') {
...
export function getScopeObject (scope) {
    const scopeObject = geval(scope)
    const constructorName = scopeObject && scopeObject.constructor ? scopeObject.constructor.name : null
    return {
        result: {
            className: constructorName,
            description: constructorName,
            objectId: '{"injectedScriptId":1,"id":1}',
            type: typeof scopeObject
        }
    }
}
