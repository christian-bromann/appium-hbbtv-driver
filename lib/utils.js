...
function getDescription (content) {
    content = content.match(/<meta [^>]*>/g) || []
    content = content.filter((metaTag) => metaTag.indexOf('name="description"') > -1)[0]

    if (!content) {
        return
    }

    content = content.slice(content.indexOf('content="') + 9)
    return content.slice(0, content.indexOf('"'))
}

function getIpAddress (iface, family = 'ipv4') {
...
async function getDeviceConfig () {
    ...
    const { deviceName, platformVersion, platformName, resolution, model, modelName } = await getDeviceInformation()
    return {
        capabilities: [{
            deviceName,
            platformVersion,
 ...
 */
async function isAlreadyRegistered (host, port, uuid) {
    try {
...
export {
    limit, errors, getDomain, hasGzipEncoding, getIpAddress, readConfig,
    writeConfig, getDescription, getDeviceInformation, getDeviceConfig,
    isAlreadyRegistered
}
