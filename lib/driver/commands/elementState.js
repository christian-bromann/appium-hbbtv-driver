...
export function isElementSelected ({ sessionId, elementId }) {
    ...
    if (elem.tagName === 'input' && elem.type.match(/(checkbox|radio)/i)) {
        return this.socket.emit('isElementSelected', elem.checked)
    }
    ...
    if (elem.tagName.toLowerCase === 'option') {
        return this.socket.emit('isElementSelected', elem.selected)
    }
     ...
     *  -> False.
     */
    return this.socket.emit('isElementSelected', false)
}
...
export function getElementAttribute ({ sessionId, elementId, attributeName }) {
    ...
    if (attribute === '' || attribute === attributeName) {
        return this.socket.emit('getElementAttribute', true)
    }
     ...
     */
    return this.socket.emit('getElementAttribute', attribute)
}
...
export function getElementProperty ({ sessionId, elementId, propertyName }) {
     ...
     */
    return this.socket.emit('getElementProperty', elem[propertyName] || null)
}
...
export function getElementCSSValue ({ sessionId, elementId, propertyName }) {
     ...
     * else let it be "".
     */
    return this.socket.emit('getElementCSSValue', window.getComputedStyle(elem)[propertyName])
}
...
export function getElementText ({ sessionId, elementId }) {
     ...
     */
    return this.socket.emit('getElementText', elem.innerText)
}
...
export function getElementTagName ({ elementId }) {
     ...
     */
    return this.socket.emit('getElementTagName', elem.tagName)
}
...
export function getElementRect ({ sessionId, elementId }) {
     ...
     */
    this.socket.emit('getElementRect', {
        /**
...
export function isElementEnabled () {
    ...
    this.socket.emit('isElementEnabled', true)
}
