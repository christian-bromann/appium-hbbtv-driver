...
export function execute ({script, args, sessionId}) {
    ...
    try {
         ...
         */
        return this.respond('execute', { sessionId, error })
    }

    /**
     * Otherwise let value be result’s data.
     */
    this.respond('execute', { sessionId, value })
}

export function executeAsync ({script, args, sessionId}) {
    window._asyncCallback = (value) => {
        this.respond('executeAsync', { sessionId, value })
    }
    ...
    try {
        ...
        return this.respond('executeAsync', { sessionId, error })
    }
}
