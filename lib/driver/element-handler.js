...
export default class ElementHandler {
    ...
    find ({ using, value }, fromElement) {
        ...
        case 'css selector':
            ...
            break
        case 'link text':
            elements = this.findByXPath(`//a[text()="${value}"]`, root)
            break
        case 'partial link text':
            elements = this.findByXPath(`//a[contains(text(),"${value}")]`, root)
            break
        case 'xpath':
        ...
        const result = elements.map((element) => {
             ...
             */
            if (this.elements.filter((e) => e === element).length) {
                return
            }
