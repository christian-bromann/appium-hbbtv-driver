...
class HbbTVDriver extends BaseDriver {
    ...
    async request (cmd, args = {}, expectResponse = true) {
         ...
         */
        if (!expectResponse) {
            this.proxy.socket.emit(cmd, Object.assign({ sessionId: this.sessionId }, args))
            logger.debug(`No response for ${cmd} expected`)
            return
        }

        return await new Promise((resolve, reject) => {
            setTimeout(reject, COMMAND_TIMEOUT)
            this.proxy.socket.once(cmd, (data) => {
                /**
                ...
                if (data.error) {
                    ...
                    return logger.errorAndThrow(data.error.message)
                }
