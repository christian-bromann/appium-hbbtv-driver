...
class HbbTVDriver extends BaseDriver {
    ...
    async request (cmd, args = {}, expectResponse = true) {
         ...
         */
        if (!expectResponse) {
            webdriver.emit(cmd, Object.assign({ sessionId: this.sessionId }, args))
        ...
        return await new Promise((resolve, reject) => {
            ...
            webdriver.once(`${cmd}Result`, (data) => {
                if (data && typeof data.error === 'string') {
                    const error = errors[data.error] ? new errors[data.error]() : new errors.UnknownError()
                    return reject(error)
                }
