...
import { limit, errors, getDeviceConfig, writeConfig, isAlreadyRegistered, readConfig } from './utils'
...
class HbbTVDriver extends BaseDriver {
    ...
    async start (caps, sessionId) {
        ...
        let debuggerAddress = caps.debuggerAddress
        if (!debuggerAddress) {
            debuggerAddress = 'localhost:9222'
            const page = this.devtools.backend.addPage({
                uuid: sessionId,
                hostname: `${process.env.PROXY_NETWORK_ADDRESS}:9222`
            })
            this.devtools.proxy.uuid = sessionId
            this.devtools.proxy.page.trigger('Page', { method: 'reload', params: { ignoreCache: true } })
            this.devtools.proxy.page = page
        }

        const remoteDebuggingPageUrl = `ws://${debuggerAddress}/devtools/page/${sessionId}`
