...
export default class Proxy {
    constructor (host = process.env.PROXY_HOST, port = 8080) {
        this.host = host
        this.port = port
        this.app = express()
        ...
        this.piIPAddress = host || getIpAddress('eth0') || getIpAddress('wlan0')
    ...
    async run () {
        ...
        this.server = this.app.listen(this.port,
            () => this.log.debug('Started proxy server')
        )
    }
    ...
    debuggerJson (req, res) {
        ...
        return res.json({
            ...
            devtoolsUrl: `http://${this.piIPAddress}:${this.port}/inspector.html`,
    ...
    async preflightCheck () {
         ...
         */
        await new Promise((resolve, reject) => dns.lookup('http://8.8.8.8', (err) => {
            if (err) {
                return reject(new Error('Couldn\'t connect to the internet: ' + err.message))
            }
            resolve()
        }))
