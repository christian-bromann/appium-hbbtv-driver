...
export default class Proxy {
    ...
    requestIdMiddleware (req, res, next) {
        ...
        /**
         * apply requestId to all request where requestId is not set or set new
         * requestId for requests with same request hosts
         */
        if (!req.cookies.requestId || req.cookies.requestIdHost === target) {
        ...
        const requestId = req.cookies.requestId.split('.')[0]

        if (!subIds[requestId]) {
            subIds[requestId] = 1
        }

        const subId = ++subIds[requestId]
        const newRequestId = `${requestId}.${subId}`
